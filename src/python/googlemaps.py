...
class GoogleMapsScraper:
    ...
    def __parse(self, review):
        ...
        try:
            ...
            # Convert the numeric rating to star representation
            if rating is not None and 1 <= rating <= 5:
                stars = '★' * int(rating) + '☆' * (5 - int(rating))  # ★ for filled stars, ☆ for empty stars
            else:
                stars = None
        except Exception as e:
            rating = None
            stars = None  # Ensure stars is also set to None on error
        ...
        try:
            # TODO: Subject to changes
            image = review.find('img', class_='NBa7we')
            reviewer_image = image['src']
        except Exception as e:
            reviewer_image = None
        ...
        item['reviewer_image'] = reviewer_image
